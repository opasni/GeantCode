...
void MyPhysicsList::InverseBeta()
{
  
  G4ProcessManager * aProcMan = 0;

  theInverseBetaProcess = new InverseBetaProcess;
  theElectroReaction = new InverseBetaModel;
  
  aProcMan = G4Electron::Electron()->GetProcessManager();
  theInverseBetaProcess->RegisterMe(theElectroReaction);
  theInverseBetaProcess->MultiplyCrossSectionBy(pow(10,13));

  aProcMan->AddDiscreteProcess(theInverseBetaProcess);
