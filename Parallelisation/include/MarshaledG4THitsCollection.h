...
void marshal1() {
    ...
    //Now just copy "get" functions here
    {
        int copy_off = 0;
        int elementNum;
         elementNum = ((G4THitsCollection<HadCalorimeterHit>*)param)->entries(); 
        memcpy( msh_cursor+copy_off, &elementNum,sizeof(int));
        copy_off += sizeof(int);
        for(int index=0;index<elementNum;index++){
           HadCalorimeterHit*   anElement;
           anElement = (*((G4THitsCollection<HadCalorimeterHit>*)param))[index]; 
           MarshaledHadCalorimeterHit   marEle(anElement);
...
void unmarshal1() {
    ...
    //Now copy the setspec here
    {
       ...
       copy_off += sizeof(int);
       for(int index=0;index<elementNum;index++){
         MarshaledHadCalorimeterHit   marEle(msh_cursor+copy_off);
         HadCalorimeterHit*   anElement = (HadCalorimeterHit*  )marEle.unmarshal();
         copy_off += marEle.getBufferSize();
         ((G4THitsCollection<HadCalorimeterHit>*)param)->insert((HadCalorimeterHit*)anElement); 
       }
